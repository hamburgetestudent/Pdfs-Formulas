...
"""
    ...
    print("Testing Parser with Full Input...")
    parsed = app.parse_input(input_text)
    
    if not parsed:
        print("FAILED: Parser returned empty/None")
        return

    print(f"Parsed {len(parsed)} sections.")
    for p in parsed:
        print(f"Section: {p['title']} - Rows: {len(p['df'])}")
        
    print("Testing PDF Generation...")
    try:
        generate_pdf(parsed, "manual_input_test.pdf")
        if os.path.exists("manual_input_test.pdf"):
            print("SUCCESS: PDF generated as manual_input_test.pdf")
        else:
            print("FAILED: PDF file not found.")
    except Exception as e:
        print(f"FAILED: PDF generation threw exception: {e}")
